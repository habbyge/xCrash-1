...
static UInt32
LitEnc_Matched_GetPrice(const CLzmaProb* probs, UInt32 symbol, UInt32 matchByte, const CProbPrice* ProbPrices) {
  UInt32 price = 0;
  UInt32 offs = 0x100;
  symbol |= 0x100;
  do {
...
MY_NO_INLINE static void MY_FAST_CALL LenPriceEnc_UpdateTable(
  ...
  UInt32* prices = p->prices[posState];
  {
    ...
    SetPrices_3(probs + kLenNumLowSymbols, a + GET_PRICEa_0(enc->low[kLenNumLowSymbols]), prices + kLenNumLowSymbols,
                ProbPrices);
    ...
    */
    ...
    if (newLen >= startLen) {
      ...
      for (len = /*2*/ startLen;; len++) {
        ...
        {
          COptimal* opt;
          unsigned lenToPosState = len - 2;
          lenToPosState = GetLenToPosState2(lenToPosState);
          if (dist < kNumFullDistances)
